...
src_install() {
	${BJAM} release backtrace-install link=$LINK --prefix=${D}/usr || die "install failed"
	if use cxx
	then
		${BJAM} release backtracexx-install link=$LINK --prefix=${D}/usr || die "C++ library version installation failed"
	fi
}
